// Sprint 10 §A-V-Idempotent — two `handleValidateFinding` calls run
// concurrently against the same candidate via Promise.all. Exactly one
// `findings` row at end. Both invocations return ack.
...
describe.skipIf(!hasDatabaseUrl())('validator :: idempotent-parallel (A-V-Idempotent)', () => {
  let fx: DbFixture;
  let tenantId: string;
  ...
  test('two parallel handlers for same candidate → exactly 1 findings row + both ack', async () => {
    await withLab(async (lab) => {
      ...
      await fx.db
        ...
        .execute();
      const projectId = await seedProject(fx, { tenantId, name: 'P-idem' });
      ...
      const env = (jobId: string): JobEnvelope => ({
        jobId,
        tenantId,
        projectId,
        assessmentId,
        kind: 'validate.finding',
        idempotencyKey: `validate:idem:${jobId}`,
      ...
      // Two independent handlers (each builds its own driver) run in parallel.
      const depsA = buildValidatorHandlerDeps({ db: fx.db, storage, buildScope });
      const depsB = buildValidatorHandlerDeps({ db: fx.db, storage, buildScope });
      const [outA, outB] = await Promise.all([
        handleValidateFinding(depsA, env(uniqUuid())),
        handleValidateFinding(depsB, env(uniqUuid())),
      ]);
      expect(outA.kind).toBe('ack');
      expect(outB.kind).toBe('ack');
