// Sprint 10 §A-V-AlertOnly — driver returns alert hit but zero DOM/console
// nonce echoes. Status `inconclusive`, NO findings row,
// `validation.inconclusive` audit emitted.
...
describe.skipIf(!hasDatabaseUrl())('validator :: alert-only inconclusive (A-V-AlertOnly)', () => {
  let fx: DbFixture;
  let tenantId: string;
  ...
  test('alert hit but no DOM/console nonce echo → inconclusive, NO findings row', async () => {
    await withLab(async (lab) => {
      ...
      await fx.db
        ...
        .execute();
      const projectId = await seedProject(fx, { tenantId, name: 'P-alert' });
      ...
      // forceAlertOnly so the driver always reports alert true while
      // /healthz returns no nonce echo → weak proof.
      const deps = buildValidatorHandlerDeps({
        db: fx.db,
        storage,
        buildScope,
        driverDeps: { forceAlertOnly: true },
      });
      ...
      const env: JobEnvelope = {
        ...
        idempotencyKey: 'validate:alert-1',
      ...
      expect(after?.reason).toBe('alert_only_weak_proof');
    });
  });
});
