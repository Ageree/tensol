...
4b. **Knowledge Graph Persistence (HARD)** [TENSOL OVERRIDE]: After writing each `findings/FIND-{NNN}.md`, IMMEDIATELY also call `kg_add_node(kind="vulnerability", label=..., props=...)` so the downstream validation pass (verifier agent) can pick it up. The KG node and the markdown file are TWO FACES of the same finding — they MUST be created together. Verifier reads ONLY from the KG; markdown alone is invisible to validation.

   Required `props` JSON fields (props is a JSON STRING, not a dict):
   - `key`: deterministic dedup id, format `"<vuln_class>:<target_url>:<parameter>"` (e.g. `"sqli:https://shop.com/products:id"`)
   - `severity`: one of `"critical"`, `"high"`, `"medium"`, `"low"`
   - `vuln_class`: short class id, one of `"sqli"`, `"xss"`, `"ssti"`, `"lfi"`, `"rce"`, `"ssrf"`, `"idor"`, `"info_disclosure"`, `"broken_auth"`, `"xxe"`, `"open_redirect"`, `"csrf"`, `"misconfig"`, `"default_creds"`
   - `target_url`: full URL where vuln was observed
   - `parameter`: input parameter name if applicable (omit field if not)
   - `evidence_pointer`: relative path to the FIND-{NNN}.md you just wrote
   - `cwe`: optional list of CWE ids, e.g. `["CWE-89"]`
   - `description`: 1-2 sentence operator summary

   Example call:
   ```
   kg_add_node(
     kind="vulnerability",
     label="SQLi in /products?id=",
     props='{"key":"sqli:https://target.com/products:id","severity":"high","vuln_class":"sqli","target_url":"https://target.com/products","parameter":"id","evidence_pointer":"findings/FIND-001.md","cwe":["CWE-89"],"description":"UNION-based SQLi via id parameter; time-delay confirmed."}'
   )
   ```

   If `kg_add_node` returns an error, log it in the finding's evidence section but DO NOT retry more than once — the markdown file is the durable record and the orchestrator will route around the KG miss.
