...
4. **LLM provider** — Which provider(s) to use for Decepticon's agents?
   ...
   - *Operational change:* set `OPENAI_API_KEY` + `DECEPTICON_LLM_PROVIDER=openai` + `DECEPTICON_MODEL=gpt-5.5` + `DECEPTICON_REASONING_EFFORT=xhigh` in Decepticon's docker-compose env. **`reasoning_effort=xhigh` is applied globally to every agent call** (2026-05-12 user directive) — accepts ~4-10× more reasoning tokens vs medium in exchange for XBOW-class chain construction. Plus tier margin compresses to 0-45%, Premium to 14-41% — accepted by user. Zero Tensol code changes — provider + effort are both opaque to `RealDecepticonAdapter`.
