 ...
 * further and reimplements the *server's* verifier shape (parse `t=N, v1=hex`,
 * recompute HMAC, timing-safe compare) — so any drift on EITHER side surfaces
 * here as a failing assertion before it can reach production.
 *
...
describe("webhook contract: Juice-Shop-shape payload (realistic body)", () => {
  test("multi-finding completion body round-trips", () => {
    const body = JSON.stringify({
      ...
      evidence_archive_url: "s3://tensol-evidence/01JJCSHPTESTSCANRDER000001.tar.gz",
      findings: [
        {
...
describe("webhook contract: server-side header parser tolerance", () => {
  // The signer emits `t=N, v1=hex` (one space after comma). The server's
  ...
  test("reversed-order `v1=hex, t=N` accepted (server tolerance)", () => {
    const body = '{"a":1}';
    const r = signWebhook({ secret: SECRET, body, timestamp: GOLDEN_TS });
    const match = r.signature.match(/^t=(\d+), v1=([0-9a-f]+)$/);
    expect(match).not.toBeNull();
    const reversed = `v1=${match![2]}, t=${match![1]}`;
    expect(verifyWebhookServerSide(SECRET, reversed, body)).toBe(true);
  });

  test("uppercase hex `V1=ABCD` accepted (server normalises to lowercase)", () => {
    const body = '{"a":1}';
    const r = signWebhook({ secret: SECRET, body, timestamp: GOLDEN_TS });
