  ...
  return (
    <div data-screen-label="10 App — findings">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <Eyebrow>// 11 findings · 6 confirmed · 1 candidate · 4 fixed</Eyebrow>
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 500, fontSize: 38, letterSpacing: '-0.02em', margin: '6px 0 0' }}>{t.fTitle}</h1>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 14.5, color: 'var(--fg-2)', margin: '6px 0 0' }}>{t.fSub}</p>
        </div>
      ...
      {/* Filters */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '14px 18px', border: '1px solid var(--fg)', marginBottom: 0, background: 'var(--bg)', borderBottom: 'none' }}>
        <Eyebrow>{t.fFilters}</Eyebrow>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Mono size={11} color="var(--fg-3)">severity:</Mono>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <Mono size={11} color="var(--fg-3)">status:</Mono>
          <Segmented size="sm" value={statusFilter} onChange={setStatusFilter} options={[
            { value: 'all', label: 'all' }, { value: 'confirmed', label: 'confirmed' }, { value: 'candidate', label: 'candidate' }
          ]} />
        </div>
      </div>

      {/* Table */}
      <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid var(--fg)', background: 'var(--bg)' }}>
        <thead>
          <tr style={{ background: 'var(--fg)', color: 'var(--bg)' }}>
            {[t.fColTitle, t.fColSev, t.fColConf, t.fColAsset, t.fColAss, t.fColStatus, ''].map((h, i) => (
              <th key={i} style={{ textAlign: 'left', padding: '10px 14px', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {filtered.map(f => (
            <tr key={f.id} style={{ borderTop: '1px solid var(--line-soft)', cursor: 'pointer' }}
                onClick={() => { setOpenF(f); }}>
              <td style={{ padding: '12px 14px' }}>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 13.5, color: 'var(--fg)' }}>{f.title}</div>
                <Mono size={10} color="var(--fg-3)" style={{ marginTop: 2 }}>{f.endpoint}</Mono>
              </td>
              <td style={{ padding: '12px 14px' }}><SeverityChip sev={f.sev} size="sm" /></td>
              <td style={{ padding: '12px 14px' }}><Mono size={11} color="var(--fg-2)">{f.conf}</Mono></td>
              <td style={{ padding: '12px 14px' }}><Mono size={11} color="var(--fg)">{f.asset}</Mono></td>
              <td style={{ padding: '12px 14px' }}><Mono size={10} color="var(--fg-3)">{f.assessment}</Mono></td>
              <td style={{ padding: '12px 14px' }}><StatusChip status={f.status} tone={f.status === 'confirmed' ? 'inverse' : 'muted'} size="sm" /></td>
              <td style={{ padding: '12px 14px', textAlign: 'right' }}><Mono size={14} color="var(--fg-3)">▸</Mono></td>
            </tr>
          ))}
          {filtered.length === 0 && (
