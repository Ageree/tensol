  ...
  return (
    <div data-screen-label="11 App — reports">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <Eyebrow>// 4 reports · 3 delivered · 1 draft</Eyebrow>
          <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 500, fontSize: 38, letterSpacing: '-0.02em', margin: '6px 0 0' }}>{t.rTitle}</h1>
          <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 14.5, color: 'var(--fg-2)', margin: '6px 0 0' }}>{t.rSub}</p>
        </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid var(--fg)', background: 'var(--bg)' }}>
        <thead>
          <tr style={{ background: 'var(--fg)', color: 'var(--bg)' }}>
            {[t.rColProject, t.rColAss, t.rColType, 'template', t.rColDate, t.rColStatus, ''].map((h, i) => (
              <th key={i} style={{ textAlign: 'left', padding: '10px 14px', fontFamily: 'monospace', fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500 }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {TENSOL_DATA.reports.map(r => (
            <tr key={r.id} style={{ borderTop: '1px solid var(--line-soft)' }}>
              <td style={{ padding: '12px 14px' }}><Mono size={12}>{r.project}</Mono></td>
              <td style={{ padding: '12px 14px' }}><Mono size={11.5} color="var(--fg-2)">{r.assessment}</Mono></td>
              <td style={{ padding: '12px 14px' }}><StatusChip status={{ tech: 'technical pentest', exec: 'executive summary', comp: 'compliance mapping' }[r.type]} tone="muted" size="sm" /></td>
              <td style={{ padding: '12px 14px' }}><Mono size={11} color="var(--fg-3)">{r.template === 'gost' ? 'GOST R / FSTEC' : 'default'}</Mono></td>
              <td style={{ padding: '12px 14px' }}><Mono size={11} color="var(--fg-3)">{r.date}</Mono></td>
              <td style={{ padding: '12px 14px' }}><StatusChip status={r.status} tone={r.status === 'delivered' ? 'inverse' : 'muted'} size="sm" /></td>
              <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                  <Btn size="sm" kind="dim">PDF</Btn>
                  <Btn size="sm" kind="dim">HTML</Btn>
                  <Btn size="sm" kind="dim">JSON</Btn>
                </div>
              </td>
            </tr>
          ))}
  ...
  return (
    <div data-screen-label="12 App — settings">
      <div style={{ marginBottom: 24 }}>
        <Eyebrow>// account · {TENSOL_DATA.user.email} · tenant {TENSOL_DATA.user.tenant}</Eyebrow>
        <h1 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 500, fontSize: 38, letterSpacing: '-0.02em', margin: '6px 0 0' }}>{t.sTitle}</h1>
      </div>
      <Card>
