...
import { type Dispatcher, createRunner } from "../src/jobs/runner.ts";
...
const DISPATCH_SIGN_KEY =
	"test-e2e-dispatch-key-64-chars-cccccccccccccccccccccccccccccccc";
...
export async function startTestServer(opts?: {
	...
	const completeQueuedReview = async (payload: unknown): Promise<void> => {
		...
		if (!review)
			throw new Error(`review fixture job: review not found (${reviewId})`);
		...
		const repo = review.repoId
			? await reviewService.getRepo(review.repoId)
			: null;
		if (
	...
	const dispatcher: Dispatcher = {
		...
		send_deep_inquiry_telegram: async () => {},
	...
	const app = createApp({
		...
		reportDownloadUrl: ({ bucket, key, expiresAt, nowMs }) => {
			if (expiresAt !== null && expiresAt <= nowMs) return null;
			return {
				url: `https://storage.example/${encodeURIComponent(bucket)}/${encodeURIComponent(key)}?signed=1`,
				expiresAt: expiresAt ?? nowMs + 30 * 60 * 1000,
			};
		},
