 ...
 *
 * Usage (from server/):
 *   TENSOL_OPENROUTER_API_KEY=sk-or-v1-... bun run scripts/e2e-harness.ts
...
import { CompositeSastRunner, createCliSastRunner } from "../src/review/sast/runner.ts";
import { fileToDiffFile } from "../src/review/repo-fetch.ts";
import type { DiffFile } from "../src/review/types.ts";

const apiKey = process.env.TENSOL_OPENROUTER_API_KEY || process.env.TENSOL_REVIEW_LLM_API_KEY || "";
if (!apiKey) {
  console.error("\n✗ No OpenRouter key. Set TENSOL_OPENROUTER_API_KEY (sk-or-v1-...) and re-run.\n");
  process.exit(1);
}
const baseUrl = process.env.TENSOL_REVIEW_LLM_BASE_URL || "https://openrouter.ai/api/v1";
const auditorModel = process.env.TENSOL_HARNESS_MODEL_AUDITOR || "openai/gpt-5.5";
const debaterModel = process.env.TENSOL_HARNESS_MODEL_DEBATER || "qwen/qwen3.7-max";
...
// ---------------------------------------------------------------------------
async function resolveCounterpoint(): Promise<{ model: string; distinct: boolean }> {
  const fromEnv = process.env.TENSOL_HARNESS_MODEL_COUNTERPOINT;
  if (fromEnv && fromEnv.trim()) return { model: fromEnv.trim(), distinct: true };
  const prefer = [
  ...
  console.warn("  ⚠ no independent counterpoint model found — falling back to the auditor model.");
...
async function main() {
  ...
  console.log(`  counterpoint = ${counterpointModel}${distinct ? "" : "  (FALLBACK — not a true 3-model ensemble)"}`);
  console.log(`  recon/triage = ${reconModel}`);
  console.log(`  budget       = ${usd(budgetUsd)}  maxAuditors=${maxAuditors}\n`);
  ...
  const budget = createBudget({ ceilingUsd: budgetUsd, usdPerMTokOut: 30, usdPerMTokIn: 5 });
  ...
  const debateFired = verdicts.some((v) => v.rationaleMd.includes("Multi-model debate"));
  const contested = verdicts.filter((v) => v.rationaleMd.toLowerCase().includes("contested")).length;
  const spent = budget.spentUsd();

  console.log("---------------------------------------------------------------");
  console.log("RESULTS");
  console.log(`  recall:          ${recalled.length}/${realVulns.length} real vulns flagged`);
  for (const f of realVulns) console.log(`    ${flagged.has(f.path) ? "✓" : "✗"} ${f.label} (${f.path})`);
  console.log(`  false positives: ${falsePositives.length}/${decoys.length} decoys flagged`);
  for (const f of decoys) console.log(`    ${flagged.has(f.path) ? "✗ FP" : "✓ ok"} ${f.label} (${f.path})`);
  console.log(`  total verdicts:  ${verdicts.length}`);
  console.log(`  multi-model debate fired: ${debateFired ? "yes" : "NO"}  (contested findings: ${contested})`);
  console.log(`  spend:           ${usd(spent)} / ${usd(budgetUsd)}   elapsed: ${(elapsedMs / 1000).toFixed(1)}s`);
  console.log("---------------------------------------------------------------\n");
  ...
  if (spent <= 0) failures.push("zero spend → the harness short-circuited (models never ran)");
  if (spent > budgetUsd + 1e-6) failures.push(`spend ${usd(spent)} exceeded the ${usd(budgetUsd)} ceiling`);
  if (recalled.length < realVulns.length - 1) failures.push(`recall ${recalled.length}/${realVulns.length} below threshold (allow at most 1 miss)`);
  if (falsePositives.length > 1) failures.push(`${falsePositives.length} false positives on decoys (allow at most 1)`);
  if (!debateFired) failures.push("multi-model debate never fired (Validate stage did not run)");
  ...
  console.log("✓ E2E PASSED — multi-model agentic harness recalled the vulns, resisted decoys, debated, and stayed in budget.\n");
}
