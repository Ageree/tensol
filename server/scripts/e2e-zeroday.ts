 ...
 *
 * Usage (from server/):
 *   TENSOL_OPENROUTER_API_KEY=sk-or-v1-... bun run scripts/e2e-zeroday.ts
...
// ---------------------------------------------------------------------------
const apiKey =
  process.env.TENSOL_OPENROUTER_API_KEY || process.env.TENSOL_REVIEW_LLM_API_KEY || "";
if (!apiKey) {
  console.error(
...
const pricePerMTok = Number(process.env.TENSOL_EXPLOIT_USD_PER_MTOK_OUT || "3.75");
...
async function f1DeepResearch(): Promise<boolean> {
  ...
  const budget = createBudget({ ceilingUsd: 1_000, usdPerMTokOut: pricePerMTok }); // measure only
  ...
  const result = await runReview({ kind: "whitebox", files, mode: "deep" }, { llm });
  ...
  console.log(`  score: ${result.score0to5}/5   findings: ${result.findings.length}   est.cost: ${usd(budget.spentUsd())}`);
  ...
  for (const fx of FIXTURES) {
    ...
    if (fx.vulnerable) {
      ...
      console.log(`    ${hit ? "✓" : "✗MISS"}  VULN ${fx.path.padEnd(28)} ${fx.label}`);
    } else {
      if (hitHigh) falsePos++;
      console.log(`    ${hitHigh ? "✗FP  " : "✓"}  SAFE ${fx.path.padEnd(28)} ${fx.label}`);
    }
  }
  ...
  console.log(`  F1 ${pass ? "PASS ✓" : "FAIL ✗"}  (need ≥5/7 recall AND ≤1 decoy FP)`);
  return pass;
}
    ...
    fetch(req) {
      ...
      if (url.pathname === "/users") {
        ...
        try {
          ...
          return new Response(`SQL error: ${(e as Error).message}`, { status: 500 });
        }
      }
...
// --- T3: Path traversal ----------------------------------------------------
function startPathTraversalTarget(canary: string): { target: LiveTarget; cleanup: () => void } {
    ...
    fetch(req) {
      ...
      if (url.pathname === "/download") {
        ...
        try {
          ...
          if (!resolve(full).startsWith(resolve(served) + sep)) exploited = true;
...
return new Response(data);`,
  ...
  return { target, cleanup: () => rmSync(root, { recursive: true, force: true }) };
}
...
async function f2OneTarget(spec: F2Spec, budget: ReturnType<typeof createBudget>): Promise<{
  proven: boolean;
  ...
  const llm = createOpenRouterClient({ apiKey, baseUrl, model, jsonMode: false });
  ...
  try {
    const outcomes = await runExploitLabForFindings([finding], {
      ...
      maxIters: 4,
      budget,
      authorization: { kind: "dns-scope", domain: "localhost", scanOrderId: "zd" },
      buildContext,
    });
    ...
    if (proven) {
      const poc = (v?.pocMd ?? "").split("\n").slice(0, 24).map((l) => "      " + l).join("\n");
      console.log(poc);
    } else {
      console.log(`    evidence: ${v?.evidence ?? "(none)"}`);
    }
    if (proven && !serverConfirmed) {
      console.log("    ⚠️  lab claimed PROVEN but server saw NO real exploit — gamed marker!");
    }
    console.log(`    ${spec.name}: ${pass ? "PASS ✓" : "FAIL ✗"}`);
    return { proven, serverConfirmed, pass, status: v?.status, iters: v?.iterations, cost };
  } finally {
    target.stop();
    cleanup?.();
  }
...
async function f2ExploitLab(): Promise<boolean> {
  ...
  console.log(`  F2 ${pass ? "PASS ✓" : "FAIL ✗"}  (${passCount}/4 classes proven+confirmed; need ≥3)`);
  return pass;
}
...
async function main(): Promise<void> {
  console.log(`ZERO-DAY E2E — model=${model} baseUrl=${baseUrl} price=$${pricePerMTok}/Mtok-out`);
  let f1 = true;
  let f2 = true;
  ...
  console.log("\n=== SUMMARY ===");
  if (runF1) console.log(`  F1 Deep Research (zero-day hunt) : ${f1 ? "PASS ✓" : "FAIL ✗"}`);
  if (runF2) console.log(`  F2 Exploit Lab (multi-class)     : ${f2 ? "PASS ✓" : "FAIL ✗"}`);
  const ok = f1 && f2;
