...
function gcpCredentialCheck(): Check {
  ...
  if (!existsSync(path)) return check("gcp.credentials", false, `file not found: ${path}`);
  try {
...
function imageParts(image: string):
  | { registry: string; repo: string; tag: string }
  | null {
...
async function checkGhcrImage(image: string): Promise<Check> {
  ...
  if (!parsed) {
    return check(`image.${image}`, false, "image must include registry/repo:tag");
  }
...
async function main(): Promise<void> {
  ...
  for (const name of [
    ...
    const value = env(name);
    checks.push(check(`env.${name}`, Boolean(value), value ? "set" : "missing"));
  }
  const openrouterKey = envAny(["TENSOL_OPENROUTER_API_KEY", "OPENROUTER_API_KEY"]);
  checks.push(
    check(
  ...
  checks.push(check("env.GCP_NETWORK_NAME", true, env("GCP_NETWORK_NAME") || "default"));
  checks.push(check("env.GCP_SUBNET_NAME", true, env("GCP_SUBNET_NAME") || "default"));
  for (const name of [
    ...
    checks.push(
      check(
        `optional.${name}`,
        true,
        value ? "set" : "missing; report/evidence storage disabled",
      ),
    );
  }
