...
const STRIPE_LIVE_KEY_RE = "sk" + "_live_[A-Za-z0-9]{8,}";
const HARDCODED_SECRET_RE = new RegExp(
	`(${STRIPE_LIVE_KEY_RE}|API_TOKEN\\s*=\\s*["'][^"']{12,}|api[_-]?key\\s*=\\s*["'][^"']{12,})`,
	"i",
);
const RULES: Rule[] = [
	{
		...
		rationale:
			"User-controlled input is concatenated into a string passed to a shell `exec`. An attacker can inject shell metacharacters (`;`, `|`, `$()`) to run arbitrary commands. Reachable from the request path.",
	},
	{
		test: (s) =>
			/(SELECT|INSERT|UPDATE|DELETE)\b[\s\S]*?(["'`]\s*\+|\+\s*["'`]|\$\{)/i.test(
				s,
			),
		anchor: (l) =>
			/(SELECT|INSERT|UPDATE|DELETE)\b/i.test(l) &&
			/["'`]\s*\+|\+\s*["'`]|\$\{/.test(l),
		...
		rationale:
			"A query is built by concatenating user-controlled input instead of parameterized bindings. An attacker controls the WHERE/LIKE clause to exfiltrate or modify data.",
	},
	{
		test: (s) => HARDCODED_SECRET_RE.test(s),
		anchor: (l) => HARDCODED_SECRET_RE.test(l),
		category: "Hardcoded Credential",
		cwe: ["CWE-798"],
		cvss: SECRET,
		title: "Hardcoded secret committed to source",
		rationale:
			"A live API token is hardcoded in source and committed to version control. Anyone with repo read access obtains the secret; rotation is the only remediation.",
	},
	{
		test: (s) =>
			/path\.join\s*\([^)]*\b(name|file|path|input|req|param)/i.test(s) &&
			/readFile|createReadStream|sendFile|open\b/.test(s),
		anchor: (l) =>
			/path\.join\s*\([^)]*\b(name|file|path|input|req|param)/i.test(l),
		category: "Path Traversal",
		cwe: ["CWE-22"],
		cvss: TRAVERSAL,
		title: "Path traversal via unsanitized filename in path.join",
		rationale:
			"A user-controlled name is joined onto a base directory and read without a containment check. A `../` sequence escapes the base directory and discloses arbitrary files.",
	},
	{
		...
		rationale:
			"Dynamic input flows into `eval`, allowing arbitrary code execution in the process.",
	},
];
...
function parseCandidates(
	user: string,
): Array<{ id?: string; file: string; line?: number; snippet: string }> {
	const out: Array<{
		id?: string;
		file: string;
		line?: number;
	...
	for (let i = 1; i < parts.length; i++) {
		...
		const id = /^\s*([^\n]+)/.exec(block)?.[1]?.trim();
		const loc =
			/location:\s*(\S+?):(\d+)/.exec(block) ?? /location:\s*(\S+)/.exec(block);
		const file = loc?.[1] ?? "";
...
function parseContextFiles(
	...
	while (true) {
		...
		if (file && content !== undefined) out.push({ file, content });
	}
	return out;
}
		...
		async complete({
			...
			if (/refut|challenge/i.test(system) || /"refuted"/.test(user)) {
				return JSON.stringify({
					refuted: false,
					reason: "sink reachable from untrusted input; not a false positive",
				});
			}
			...
			const emit = (
				...
				for (const r of RULES) {
					...
					const line =
						anchorLine(content, r) ?? anchorLine(text, r) ?? fallbackLine;
					verdicts.push({
						...(id ? { candidate_id: id } : {}),
						file_path: file,
			...
			for (const c of parseCandidates(user))
				emit(c.file, c.line, c.id, c.snippet);
			for (const [file, content] of ctx)
				emit(file, undefined, undefined, content);
			return JSON.stringify({
				summary:
					verdicts.length > 0
