 ...
 *     implementation matches the schema's `jobs.type` column union extension
 *     and uses an `as never` cast because `send_deep_inquiry_telegram` is
 *     not yet in the schema's hardcoded `$type<>` literal (T102 will add it).
 */
...
import { sanitizeScopeText, type SanitizeResult } from "./sanitize.ts";
import type {
  CreateInquiryBody,
  DeepInquiryResponse,
  DeepInquiryStatus,
...
export function createDeepInquiriesService(
   ...
   *
   *  We cast the `type` to `never` because `send_deep_inquiry_telegram` is
   *  not yet in the Drizzle `$type<>` literal union (T102 will add it). The
   *  underlying SQLite column has no CHECK constraint on `type`, so any
