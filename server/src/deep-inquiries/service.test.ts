...
type JobsTableType = (typeof jobsTable.$inferSelect)["type"];
...
function validBody(
	overrides: Partial<CreateInquiryBody> = {},
): CreateInquiryBody {
	return {
		company: "Acme Corp",
...
function countJobs(db: DB, type: JobsTableType): number {
	const row = db
		.select({ c: count() })
		.from(jobsTable)
		.where(eq(jobsTable.type, type))
		.get();
	return row?.c ?? 0;
}
...
describe("createInquiry — anonymous", () => {
	...
	test("enqueues send_deep_inquiry_telegram job", async () => {
		...
		if (!jobRow) {
			throw new Error("Expected send_deep_inquiry_telegram job to be inserted");
		}
