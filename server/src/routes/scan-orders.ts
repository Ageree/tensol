...
const DEFAULT_SCAN_ORDER_LIST_LIMIT = 100;
const MAX_SCAN_ORDER_LIST_LIMIT = 500;

function parseLimit(raw: string | undefined): number {
	if (raw === undefined) return DEFAULT_SCAN_ORDER_LIST_LIMIT;
	const parsed = Number.parseInt(raw, 10);
	if (!Number.isFinite(parsed)) return DEFAULT_SCAN_ORDER_LIST_LIMIT;
	return Math.max(1, Math.min(MAX_SCAN_ORDER_LIST_LIMIT, parsed));
}
