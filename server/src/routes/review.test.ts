...
const fakeAuth: MiddlewareHandler<{ Variables: AuthVariables }> = async (
	c,
	next,
) => {
...
describe("GET /v1/review/:id + list + repos", () => {
	...
	test("list endpoints honor limit query params", async () => {
		...
		const reviewsRes = await app.request("/?limit=2");
		expect(reviewsRes.status).toBe(200);
		const reviews = (await reviewsRes.json()) as Array<{ pr_number: number }>;
		expect(reviews.map((r) => r.pr_number)).toEqual([5, 4]);

		const reposRes = await app.request("/repos?limit=3");
...
describe("POST /v1/review/whitebox (async)", () => {
	test("enqueues a whitebox_scan job and returns 202", async () => {
		...
		if (!jobRow) throw new Error("expected whitebox job row");
	...
	test("F1: mode='deep' is rejected (422) when research is disabled", async () => {
		...
		process.env.TENSOL_RESEARCH_ENABLED = undefined;
		try {
			const db = freshMemDb();
			const { app } = makeReviewApp(db);
			const res = await app.request("/whitebox", {
			...
			else process.env.TENSOL_RESEARCH_ENABLED = undefined;
		}
	});
...
describe("POST /v1/review/github/webhook", () => {
	...
	test("#7 dedup row + review + job commit atomically (no orphan dedup on a crash path)", async () => {
		...
		const dedupRow = db.$client
			.query("SELECT * FROM webhook_dedup WHERE dedup_key = ?")
			.get("d-atomic");
		...
		const reviewCount = db.$client
			.query("SELECT COUNT(*) AS n FROM reviews")
			.get() as { n: number };
	...
	test("@sthrip review on a repo NOT connected via its installation is ignored (no job, cross-tenant safe)", async () => {
		...
		const jobs = db
			.select()
			.from(jobsTable)
			.where(eq(jobsTable.type, "pr_review"))
			.all();
...
describe("PATCH /v1/review/repos/:id/settings", () => {
	...
	test("unknown repo_id gets 403 (hides existence)", async () => {
		...
		const { app } = makeReviewApp(db);
		const res = await app.request(
			"/repos/01JZZZUNKNOWN0000000000000/settings",
			{
				method: "PATCH",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ enabled: true }),
...
describe("mount isolation (webhook un-gated under the authed /v1/review prefix)", () => {
	test("webhook path reaches its handler; the authed detail path is gated", async () => {
		...
		expect(((await wh.json()) as { error: string }).error).toBe(
			"invalid_signature",
		);
