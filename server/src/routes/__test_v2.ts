...
const SeedReviewRepoBody = z.object({
	user_id: z.string().min(1).max(64),
	owner: z.string().min(1).max(100).optional(),
	name: z.string().min(1).max(100).optional(),
	installation_id: z.string().min(1).max(64).optional(),
	enabled: z.boolean().optional(),
	covered_branches: z.array(z.string().min(1).max(128)).max(20).optional(),
...
export function createTestV2Router(deps: CreateTestV2RouterDeps): Hono {
	...
	app.post("/seed-session", async (c) => {
		...
		return c.json({ session_id: sessionId, user_id: effectiveUserId }, 200);
	});
	...
	app.post("/expire-dns-verify", async (c) => {
		...
		return c.json({ ok: true, dns_verify_requested_at: backdated }, 200);
	});
	...
	// -------------------------------------------------------------------------
	// POST /seed-review-repo
	//   Fabricate a connected GitHub App installation + tracked repo for
	//   Playwright coverage of /repositories, whitebox launch, and PR webhook
	//   trigger paths. This mirrors post-connect state, without calling GitHub.
	// -------------------------------------------------------------------------
	app.post("/seed-review-repo", async (c) => {
		const raw = await c.req.json().catch(() => null);
		const v = badBody(SeedReviewRepoBody, raw);
		if (!v.ok) return c.json(v.payload, 422);

		const { user_id } = v.data;
		const owner = v.data.owner ?? "octo-org";
		const name = v.data.name ?? "juice-shop";
		const installationId = v.data.installation_id ?? `inst_${newIdFn()}`;
		const coveredBranches = v.data.covered_branches ?? ["main"];
		const ts = nowFn();
		...
		const existingInstallation = db
			...
			.where(
				and(
					eq(installationsTable.scm, "github"),
					eq(installationsTable.installationId, installationId),
				),
			)
			.get();
		const installationRowId = existingInstallation?.id ?? newIdFn();
		...
		if (existingInstallation) {
			db.update(installationsTable)
				...
				.where(eq(installationsTable.id, installationRowId))
				.run();
		} else {
			db.insert(installationsTable)
				.values({
		...
		const existingRepo = db
			.select({ id: reviewReposTable.id })
			.from(reviewReposTable)
			.where(
				and(
					eq(reviewReposTable.scm, "github"),
			...
			.get();
		const repoId = existingRepo?.id ?? newIdFn();
		const repoValues = {
			userId: user_id,
			installationId,
			owner,
			name,
			defaultBranch: "main",
			coveredBranchesJson: JSON.stringify(coveredBranches),
			status: "active" as const,
			enabled: v.data.enabled === false ? 0 : 1,
			statusCheckEnabled: v.data.status_check_enabled === false ? 0 : 1,
			mergeBlockOnCritical: v.data.merge_block_on_critical === true ? 1 : 0,
			lastReviewId: null,
			installationRowId,
			updatedAt: ts,
		};
		...
		if (existingRepo) {
			db.update(reviewReposTable)
				.set(repoValues)
				.where(eq(reviewReposTable.id, repoId))
				.run();
		} else {
			db.insert(reviewReposTable)
				.values({
					id: repoId,
					scm: "github",
					rulesMd: null,
