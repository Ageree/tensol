...
const DEFAULT_SCAN_EVENTS_LIMIT = 500;
const MAX_SCAN_EVENTS_LIMIT = 1_000;
const DEFAULT_SCAN_FINDINGS_LIMIT = 250;
const MAX_SCAN_FINDINGS_LIMIT = 500;
...
const SEVERITY_RANK_SQL = sql<number>`CASE ${findingsTable.severity}
...
function parseLimit(
	...
	if (raw === undefined) return defaults.value;
	const parsed = Number.parseInt(raw, 10);
	if (!Number.isFinite(parsed)) return defaults.value;
	return Math.max(1, Math.min(defaults.max, parsed));
}
...
export function createScansRouter(
	...
	app.get("/:id/events", (c) => {
		...
		const limit = parseLimit(c.req.query("limit"), {
			value: DEFAULT_SCAN_EVENTS_LIMIT,
			max: MAX_SCAN_EVENTS_LIMIT,
		});
			...
			.where(
				sinceMs > 0
					? and(
							...
							gt(scanEventsTable.createdAt, sinceMs),
						)
					: eq(scanEventsTable.scanId, scanId),
			)
	...
	app.get("/:id/findings", (c) => {
		...
		if (!scan) return c.json(NOT_FOUND, 404);
		const limit = parseLimit(c.req.query("limit"), {
			value: DEFAULT_SCAN_FINDINGS_LIMIT,
			max: MAX_SCAN_FINDINGS_LIMIT,
		});
		...
		const rows = db
			...
			.orderBy(
				SEVERITY_RANK_SQL,
				asc(findingsTable.createdAt),
				asc(findingsTable.id),
			)
			.limit(limit)
			.all();
	...
	app.get("/:id/findings/:findingId", (c) => {
		...
		const row = db
			...
			.from(findingsTable)
			.where(
				and(eq(findingsTable.id, findingId), eq(findingsTable.scanId, scanId)),
			)
			.get();
