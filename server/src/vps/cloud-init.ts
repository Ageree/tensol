...
const DEFAULT_AGENT_IMAGE =
	"ghcr.io/ageree/tensol-vps-agent:002-blackbox-mvp-latest";
...
function assertNoDelimiterCollision(
	...
	if (re.test(payload)) {
		throw new Error(
			`cloud-init: heredoc delimiter ${delim} collides with content of ${label}. Pick a different delimiter or strip the conflicting line from the source.`,
		);
	}
}
