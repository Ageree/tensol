...
const juiceShopFixture = {
  ...
  findings: [
    ...
    },
    {
      ...
      body_md: "# [CRITICAL] Hardcoded admin credentials\n\nAdmin login: admin/admin123",
...
describe("RawYamlFrontmatterSchema", () => {
  ...
  it("preserves unknown keys (forward-compat per webhook.md)", () => {
    ...
    expect((parsed as Record<string, unknown>).future_field_we_dont_know_yet).toBe(
      "some-value",
    );
  });
  ...
  it.each([
    ...
    ["severity"],
    ["title"],
  ])("rejects missing required key %s", (key) => {
...
describe("FindingFromAgentSchema", () => {
  ...
  it.each([
    ...
    ["body_md"],
    ["evidence_keys"],
  ])("rejects missing top-level field %s", (key) => {
    const base = { ...juiceShopFixture.findings[0] } as Record<string, unknown>;
    delete base[key];
