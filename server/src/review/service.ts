...
export interface ReviewService {
	...
	resolveThreadByFingerprint(
		args: ResolveThreadByFingerprintArgs,
	): Promise<void>;
...
export function createReviewService(
		...
		if (existing) {
			db.update(reviewReposTable)
				.set({
					...
					rulesMd: args.rulesMd === undefined ? existing.rulesMd : args.rulesMd,
					updatedAt: ts,
				})
	...
	return {
		...
		async listReposByUser(userId, opts) {
			return db
				.select()
				.from(reviewReposTable)
				.where(eq(reviewReposTable.userId, userId))
				.orderBy(desc(reviewReposTable.createdAt), desc(reviewReposTable.id))
		...
		async finalizeReview(id, result) {
			...
			if (review.repoId !== null) {
				db.update(reviewReposTable)
					.set({ lastReviewId: id, updatedAt: ts })
					.where(eq(reviewReposTable.id, review.repoId))
					.run();
			}
			await emit(
		...
		async listReviewsByRepo(repoId, opts) {
			return db
				.select()
				.from(reviewsTable)
		...
		updateRepoSettings: (args) => installationMethods.updateRepoSettings(args),
