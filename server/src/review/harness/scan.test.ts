...
const session = (auditor: ChatTransport): HarnessSession => ({
	models: {
		auditor,
		debater: auditor,
		counterpoint: auditor,
		recon: idleLlm,
		triage: idleLlm,
	},
...
const candidates: Candidate[] = [
	{ id: "c", filePath: "a.ts", source: "llm", hint: "h" },
];
...
test("fans out over lenses and dedups identical findings", async () => {
	const transport = {
		chat: async () => ({ content: okVerdict("a.ts"), toolCalls: [] }),
	};
...
test("caps the number of auditors at maxAuditors", async () => {
	...
	await runScan({
		units: [
			unit("injection", "a.ts"),
			unit("authentication-failures", "b.ts"),
			unit("cryptographic-failures", "c.ts"),
		],
		files,
		candidates,
		threatModelMd: "",
