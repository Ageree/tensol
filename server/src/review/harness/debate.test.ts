...
const sess = (
	debater: ChatTransport,
	counterpoint: ChatTransport,
	distinct = true,
): HarnessSession => ({
	models: {
		debater,
...
test("R1 refutes (high) → dropped", async () => {
	const r = await debate({
		finding,
		files: [],
		session: sess(refute(true), refute(false)),
		tools: [],
		maxRounds: 3,
	});
...
test("both fail to refute → high credibility, survives, annotated", async () => {
	const r = await debate({
		finding,
		files: [],
		session: sess(refute(false), refute(false)),
		tools: [],
		maxRounds: 3,
	});
...
test("R1 fails, R2 refutes → contested downgrade, survives at low confidence", async () => {
	const r = await debate({
		finding,
		files: [],
		session: sess(refute(false), refute(true)),
		tools: [],
		maxRounds: 3,
	});
	expect(r.survived).toBe(true);
