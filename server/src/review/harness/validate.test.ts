...
const refute = (refuted: boolean) => ({
  chat: async () => ({
    content: JSON.stringify({ refuted, confidence: "high", reason_md: "x", reachable: !refuted }),
    toolCalls: [],
  }),
});
...
const sess = (refuted: boolean): HarnessSession =>
  ({
    models: { debater: refute(refuted), counterpoint: refute(false), auditor: refute(false), recon: {}, triage: {} },
...
test("drops refuted, keeps survivors as plain LlmVerdict[]", async () => {
  ...
  expect((survivors[0] as unknown as Record<string, unknown>).auditorLens).toBeUndefined();
...
test("budget exhaustion keeps the finding at its ORIGINAL confidence (still scored, not demoted)", async () => {
  const exhausted: HarnessSession = {
    models: { debater: refute(false), counterpoint: refute(false), auditor: refute(false), recon: {}, triage: {} },
    modelNames: { auditor: "A", debater: "D", counterpoint: "C", recon: "R" },
    budget: {
      assertWithin() {
