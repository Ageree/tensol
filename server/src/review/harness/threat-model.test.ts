...
test("sql kind → injection lens; secret → sensitive-information-exposure", () => {
	const out = buildThreatModel({
		units: [
			unit({ kind: "sql" }),
			unit({ id: "U2", kind: "secret", filePath: "s.ts" }),
		],
		rawFindings: [],
		maxUnits: 10,
	});
...
test("git recency raises priority", () => {
	const units = [
		unit({ id: "h", filePath: "hot.ts" }),
		unit({ id: "c", filePath: "cold.ts" }),
	];
	const out = buildThreatModel({
		units,
		rawFindings: [],
...
test("SAST corroboration raises priority", () => {
	const units = [
		unit({ id: "x", filePath: "flagged.ts" }),
		unit({ id: "y", filePath: "plain.ts" }),
	];
	const raw = [
		{
			ruleId: "r",
...
test("unknown kind falls back to insecure-design", () => {
	const unknownKindUnit = {
		...unit(),
		kind: "nonsense",
	} as unknown as RoutingUnit;
	const out = buildThreatModel({
		units: [unknownKindUnit],
		rawFindings: [],
		maxUnits: 5,
	});
...
test("runReconPass returns the model summary, tolerates failure", async () => {
	const okLlm: LlmClient = { complete: async () => "## TM" };
	const ok = await runReconPass(okLlm, "summary");
	...
	const fail = await runReconPass(failingLlm, "summary");
	expect(fail).toBe("");
});
