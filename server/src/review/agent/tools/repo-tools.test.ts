...
const caps = (
	over: Partial<RepoToolCapabilities> = {},
...
const byName = (n: string, c = caps()) => {
	const tool = buildRepoAgentTools(c).find((t) => t.spec.name === n);
	if (!tool) {
		throw new Error(`Expected repo tool ${n} to be registered`);
	}
	return tool;
};

test("exposes the five repo tools", () => {
	const names = buildRepoAgentTools(caps())
		.map((t) => t.spec.name)
		.sort();
...
test("read_file returns contents", async () => {
	expect(await byName("read_file").run({ path: "src/a.ts" })).toContain(
		"export const x",
	);
});

test("read_file rejects path traversal", async () => {
	expect(await byName("read_file").run({ path: "../../etc/passwd" })).toMatch(
		/^ERROR:/,
	);
});

test("read_file rejects query-injection chars", async () => {
	expect(await byName("read_file").run({ path: "a.ts?ref=evil" })).toMatch(
		/^ERROR:/,
	);
});

test("read_file miss is not an error", async () => {
	expect(await byName("read_file").run({ path: "src/missing.ts" })).toContain(
		"file not found",
	);
});
...
test("query_reachability degrades gracefully when no client", async () => {
	expect(
		await byName("query_reachability").run({ file: "src/b.ts", line: 4 }),
	).toContain("unknown");
});
...
test("query_reachability rejects non-positive / NaN line", async () => {
	const reachability: RepoToolCapabilities["reachability"] = {
		analyze: async () => ({}),
	};
	const t = byName("query_reachability", caps({ reachability }));
