...
describe("loadConfig", () => {
	...
	test("throws when EMAIL_PROVIDER=resend but RESEND_API_KEY missing", () => {
		expect(() => loadConfig({ ...validEnv, EMAIL_PROVIDER: "resend" })).toThrow(
			/RESEND_API_KEY/,
		);
	});
	...
	test("feature gates turn ON only for explicit truthy tokens", () => {
		...
		expect(
			loadConfig({ ...validEnv, TENSOL_EXPLOIT_ENABLED: "maybe" })
	...
	test("legacy agentic whitebox flag does not alias to the MDASH harness", () => {
		const cfg = loadConfig({
			...validEnv,
			TENSOL_AGENT_WHITEBOX_ENABLED: "true",
		});
		expect(cfg.TENSOL_AGENT_WHITEBOX_ENABLED).toBe(true);
	...
	describe("GITHUB_APP_CLIENT_SECRET", () => {
		...
		test("accepts a custom secret", () => {
			const cfg = loadConfig({
				...validEnv,
				GITHUB_APP_CLIENT_SECRET: "ghs_secretvalue123",
			});
	...
	describe("STHRIP_REVIEW_CONFIDENCE_FLOOR", () => {
		...
		test("accepts 'verified'", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_REVIEW_CONFIDENCE_FLOOR: "verified",
			});
		...
		test("accepts 'high'", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_REVIEW_CONFIDENCE_FLOOR: "high",
			});
		...
		test("accepts 'medium'", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_REVIEW_CONFIDENCE_FLOOR: "medium",
			});
			expect(cfg.STHRIP_REVIEW_CONFIDENCE_FLOOR).toBe("medium");
		});
		...
		test("accepts 'low'", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_REVIEW_CONFIDENCE_FLOOR: "low",
			});
	...
	describe("STHRIP_SUPPRESS_AFTER_N_IGNORES", () => {
		...
		test("coerces string to number", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_SUPPRESS_AFTER_N_IGNORES: "5",
			});
		...
		test("accepts a positive integer override", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_SUPPRESS_AFTER_N_IGNORES: "10",
			});
	...
	describe("STHRIP_OPENGREP_RULES_DIR", () => {
		...
		test("accepts a custom path", () => {
			const cfg = loadConfig({
				...validEnv,
				STHRIP_OPENGREP_RULES_DIR: "/opt/rules/aikido",
			});
	...
	describe("TENSOL_HARNESS_* (MDASH harness)", () => {
		...
		test("enabled parses strictly via envBool", () => {
			expect(
				loadConfig({ ...validEnv, TENSOL_HARNESS_ENABLED: "true" })
					.TENSOL_HARNESS_ENABLED,
			).toBe(true);
			expect(
				loadConfig({ ...validEnv, TENSOL_HARNESS_ENABLED: "0" })
					.TENSOL_HARNESS_ENABLED,
			).toBe(false);
		});
		...
		test("counterpoint model is configurable", () => {
			const c = loadConfig({
				...validEnv,
				TENSOL_HARNESS_MODEL_COUNTERPOINT: "google/gemini-2.5-pro",
			});
