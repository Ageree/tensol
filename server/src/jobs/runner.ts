 ...
 *   start() → drains up to `maxJobsPerDrain` jobs per scheduled wake-up, so a
 *             burst of short jobs does not pay one poll interval per row.
...
import { type DB, withTx } from "../db/client.ts";
import { jobs, scans } from "../db/schema.ts";
import { ulid } from "../lib/ids.ts";
import { now as defaultNow } from "../lib/time.ts";
import type { Dispatcher, Job, JobRow, JobType, WatchdogJob } from "./types.ts";

const DEFAULT_POLL_INTERVAL_MS = 500;
const DEFAULT_MAX_ATTEMPTS = 5;
const DEFAULT_MAX_JOBS_PER_DRAIN = 25;
const MAX_JOBS_PER_DRAIN_HARD_CAP = 250;
 ...
 * the GCP-specific `spawn_scan_vm` / `teardown_scan_vm` kinds
...
const DEPRECATED_JOB_KINDS: Readonly<Record<string, string>> = {
	spawn_vps: "spawn_scan_vm",
	teardown_vps: "teardown_scan_vm",
};
...
function warnDeprecatedOnce(kind: string): void {
	...
	console.warn(
		`[deprecated] job type '${kind}' is superseded by '${replacement}' (T066). Existing handlers continue to run, but new enqueue sites must use the replacement kind.`,
	);
}
...
export interface RunnerOpts {
	...
	/** Number of ready jobs `start()` may process back-to-back before yielding
	 *  to the poll timer. `tick()` intentionally remains a single-job debugger
	 *  step. This is sequential on one DB handle; horizontal concurrency should
	 *  come from multiple runner processes/connections. */
...
export interface Runner {
	...
	enqueue<T extends Job>(job: T, opts?: { delayMs?: number }): Promise<JobRow>;
...
export function createRunner(opts: RunnerOpts): Runner {
	const {
		...
		maxJobsPerDrain: rawMaxJobsPerDrain = DEFAULT_MAX_JOBS_PER_DRAIN,
		now = defaultNow,
		signingKey,
		onError,
		watchdogIntervalMs = DEFAULT_WATCHDOG_INTERVAL_MS,
	} = opts;
	const maxJobsPerDrain = clampPositiveInt(
		rawMaxJobsPerDrain,
		DEFAULT_MAX_JOBS_PER_DRAIN,
		MAX_JOBS_PER_DRAIN_HARD_CAP,
	);
	...
	async function processClaimed(row: JobRow): Promise<void> {
		...
		if (!handler) {
			await finalizeFailure(
				row,
				new Error(
					`job ${row.id}: no handler registered for type='${handlerKey}'`,
				),
				/*permanent=*/ true,
			);
			return;
	...
	async function finalizeFailure(
		...
		if (permanent) {
			...
			const backoffMs = 2 ** row.attempts * BACKOFF_BASE_MS;
			db.update(jobs)
				.set({
					status: "pending",
					lastError: err.message,
	...
	async function drainAvailable(): Promise<JobRow[]> {
		const claimed: JobRow[] = [];
		for (let i = 0; i < maxJobsPerDrain; i += 1) {
	...
	// ---- start / stop ------------------------------------------------------
	function scheduleNext(delayMs: number = pollIntervalMs): void {
		if (stopped) return;
		timer = setTimeout(async () => {
			timer = null;
			if (stopped) return;
			const p = drainAvailable().catch(() => []);
			inFlight = p;
			let claimedCount = 0;
			try {
				claimedCount = (await p).length;
			} finally {
				inFlight = null;
			}
			if (!stopped)
				scheduleNext(claimedCount >= maxJobsPerDrain ? 0 : pollIntervalMs);
		}, delayMs);
	}
