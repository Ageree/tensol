...
function buildSvc(
  ...
  const deps = opts?.newIdSeq
    ? { ...base, newId: opts.newIdSeq }
    : base;
...
describe("createDraft", () => {
  test("happy path returns status='draft' + tier='quick'", async () => {
    ...
    expect(result.dns_verify_token).toMatch(/^tensol-verify-[0-9A-HJKMNP-TV-Z]{26}$/);
  });
  ...
  test("emits scan_order_created audit row", async () => {
    ...
    const userId = seedUser(db);
    const svc = buildSvc(db);
    await svc.createDraft(userId, { tier: "quick", primary_domain: "example.com" });
...
describe("checkDnsAndUnlock", () => {
  test("resolver match → dns_verified", async () => {
    ...
    const expectedToken = draftRow!.dnsVerifyToken;
...
describe("launchScan", () => {
  async function setupVerified(db: DB) {
    ...
    const svc = buildSvc(db, {
      dnsResolver: async () => [tokenRow!.dnsVerifyToken],
    });
    await svc.requestDnsVerify(userId, draft.id);
  ...
  test("happy path consumes free-tier + inserts scans + jobs row", async () => {
    ...
    // jobs row exists with type spawn_yandex_vm
    const jobRow = db
      .select()
      .from(jobsTable)
      .where(eq(jobsTable.type, "spawn_yandex_vm"))
      .get();
    expect(jobRow).toBeTruthy();
    expect(jobRow?.status).toBe("pending");

    // free-tier consumed
    const u = db.select().from(usersTable).where(eq(usersTable.id, userId)).get();
