	...
	db.insert(scanOrdersTable)
		.values({
			...
			attackSurfaceJson: JSON.stringify([
				{
					domain: opts?.primaryDomain ?? "example.com",
					primary: true,
					headers: [],
				},
			]),
...
describe("GET /v1/scans/:id (summary)", () => {
	...
	test("foreign user → 404 (no existence leak)", async () => {
		...
		const { scanId } = seedScanOrderAndScan(db, them.userId, {
...
describe("GET /v1/scans/:id/events (polled)", () => {
	test("happy path → 200 returns all events (no `since`)", async () => {
		...
		const first = atOrThrow(body, 0, "first scan event");
		const last = atOrThrow(body, 4, "last scan event");
		expect(first.scan_id).toBe(scanId);
		expect(first.event_type).toBe("agent_started");
		expect(last.event_type).toBe("scan_completed");
		// payload must be parsed JSON, not a string
		expect((first.payload as { step: number }).step).toBe(0);
	});
	...
	test("`since` filter returns strictly-after events", async () => {
		...
		expect(
			atOrThrow(body, 0, "first filtered event").created_at,
		).toBeGreaterThan(since);
		expect(
			atOrThrow(body, 1, "second filtered event").created_at,
		).toBeGreaterThan(since);
	});

	test("events list is bounded by default and honors limit", async () => {
		const db = freshMemDb();
		const app = buildApp({ db });
		const { cookie, userId } = seedUserAndSession(db);
		const { scanId } = seedScanOrderAndScan(db, userId);
		seedEvents(db, scanId, 520, 1_700_000_200_000);

		const defaultRes = await app.request(`/v1/scans/${scanId}/events`, {
			headers: { Cookie: cookie },
		});
		expect(defaultRes.status).toBe(200);
		const defaultBody = (await defaultRes.json()) as Array<{
			created_at: number;
		}>;
		expect(defaultBody).toHaveLength(500);

		const smallRes = await app.request(`/v1/scans/${scanId}/events?limit=12`, {
			headers: { Cookie: cookie },
		});
		expect(smallRes.status).toBe(200);
		const smallBody = (await smallRes.json()) as Array<{ created_at: number }>;
		expect(smallBody).toHaveLength(12);
		expect(
			atOrThrow(smallBody, 0, "first limited event").created_at,
		).toBeLessThan(atOrThrow(smallBody, 11, "last limited event").created_at);
	});
...
describe("GET /v1/scans/:id/findings (list)", () => {
	test("happy path → 200 returns N findings ordered by severity DESC, created_at ASC", async () => {
		...
		const first = atOrThrow(body, 0, "first finding");
		expect(Array.isArray(first.cwe)).toBe(true);
		expect(Array.isArray(first.evidence_keys)).toBe(true);
	});
	...
	test("findings list honors limit while keeping severity order", async () => {
		const db = freshMemDb();
		const app = buildApp({ db });
		const { cookie, userId } = seedUserAndSession(db);
		const { scanId } = seedScanOrderAndScan(db, userId);
		seedFindings(db, scanId, 20, 1_700_000_200_000);

		const res = await app.request(`/v1/scans/${scanId}/findings?limit=5`, {
			headers: { Cookie: cookie },
		});
		expect(res.status).toBe(200);
		const body = (await res.json()) as Array<{ severity: string }>;
		expect(body).toHaveLength(5);
		expect(body.map((f) => f.severity)).toEqual([
			"critical",
			"critical",
			"critical",
			"high",
...
describe("GET /v1/scans/:id/findings/:findingId (detail)", () => {
	test("happy path → 200 with full body_md + evidence keys", async () => {
		...
		const fid = atOrThrow(findingIds, 1, "seeded finding id");
	...
	test("foreign user → 404", async () => {
		...
		const fid = atOrThrow(findingIds, 0, "seeded finding id");
...
describe("POST /v1/scans/:id/report/regenerate", () => {
	test("happy path → 202 + enqueues render_pdf job", async () => {
		...
		const enqueued = db
			...
			.from(jobsTable)
			.where(
				and(eq(jobsTable.type, "render_pdf"), eq(jobsTable.status, "pending")),
			)
			.all();
		expect(enqueued.length).toBe(1);
		const payload = JSON.parse(
			atOrThrow(enqueued, 0, "enqueued render_pdf job").payloadJson,
		) as {
			type: string;
			scanId: string;
			reportId: string;
