...
- один облачный провайдер: `Google Cloud`
...
- **open-weights backup для compliance** — DeepSeek V4 (MIT) + K2.6 (Modified MIT) self-hostable на Google Cloud GPU; Opus 4.7 — international API только до enterprise data-residency phase
...
- Storage: с Decepticon SQLite/Neo4j → Google Cloud managed PostgreSQL + Neo4j (managed или self-hosted в K8s)
- Infra: с local Docker Compose → Google Cloud managed Kubernetes
...
- License Apache-2.0 OK, но pinned commit + own mirror в Artifact Registry обязательно
...
- LLM hosting: international API endpoints (`api.anthropic.com`, `api.deepseek.com`, `api.moonshot.ai`) → self-hosted DeepSeek V4 + K2.6 на Google Cloud GPU когда требует enterprise data-residency SLA (Opus 4.7 closed-weights — international API only, либо Anthropic on-prem deal для enterprise)
- Storage: с Decepticon SQLite + Neo4j → Google Cloud managed PostgreSQL + Neo4j
...
- Open-weights MIT → self-hostable на GCP GPU когда compliance потребует
...
| **Phase 6+ (enterprise data-residency)** | Self-host DeepSeek V4 + K2.6 на Google Cloud GPU; Opus 4.7 — Anthropic on-prem deal или alternative open-weight reasoning model | Compliance / latency драйверы |
...
**Меры:**
- Pinned upstream commit в Artifact Registry
...
**Меры:**
- Pinned upstream commit, mirror в Artifact Registry
...
- **Anthropic-Cybersecurity-Skills audit + import** — ручная проверка ~50-100 skills (web pentest + cloud + AD + threat hunting subset) на актуальность команд / качество verification steps; pinned upstream commit; mirror в Artifact Registry; index в `skill_library` table с frontmatter metadata + framework mappings
- Skill retrieval API — query by domain / tag / ATT&CK technique / keyword

### Phase 2 — Decepticon integration

- Decepticon Docker deployment в GCP K8s (dual network isolation)
...
- **Decepticon + Neo4j** — managed Neo4j на Google Cloud или self-hosted в K8s? Если self-hosted — overhead на maintenance.
...
- **Single-author bus factor** — mukul975 как single maintainer. Mitigation: own fork в Artifact Registry на случай если upstream исчезнет; периодический snapshot всего репо в archive.
...
- **Self-host triggers** — DeepSeek V4 + K2.6 self-hostable на GCP GPU. Opus 4.7 closed-weights — international API only до Anthropic on-prem deal. Решение по self-host: enterprise data-residency SLA или latency goal, не cost.
...
### Compliance / general
- **RU sanctions exposure** — Apache-2.0 / Modified MIT не блокируются. Artifact Registry mirror решает.
- **Self-host K2.6 на GCP GPU** — 8x H100 кластер ~$15-25K/мес. Включается когда ARR оправдывает.
