...
const ru: TensolDict = {
  ...
  pricing: {
    navLabel: 'Тарифы',
    eyebrow: '// ТАРИФЫ',
    title: 'Начните с оплачиваемых blackbox-кредитов. Расширяйтесь до постоянного покрытия.',
    sub: 'Starter, Team и Pro оплачиваются через OxaPay и пополняют scan credits. Для whitebox, PR review и enterprise-периметра остаётся согласование правил работы.',
    mythosPositioning:
      'Sthrip тарифицирует оплачиваемые blackbox-запуски по кредитам, а source-backed и managed-покрытие — по согласованному периметру, модели авторизации, критичным потокам и требованиям к доказательствам.',
    ...
    plans: [
      {
        name: 'Starter',
        price: '$99',
        priceAlt: '2 теста / месяц',
        unit: '/мес',
        claim: 'Self-serve blackbox-покрытие для ранних команд',
        bestFor:
          'Небольшим командам, которым нужен платный путь после бесплатной квоты и проверка сфокусированного публичного периметра.',
        depth: '2 blackbox-теста · 5 активов · 1 параллельный тест · подписанный аудит-лог',
        ctaLabel: 'Оплатить через OxaPay →',
        ctaHref: '/billing?product=starter',
      },
      {
        name: 'Team',
        price: '$299',
        priceAlt: '15 тестов / месяц',
        unit: '/мес',
        claim: 'Регулярные blackbox-проверки для продуктовых команд',
        bestFor:
          'Командам, которые проверяют несколько приложений или релиз-кандидатов каждый месяц.',
        depth: '15 blackbox-тестов · 20 активов · 4 параллельных теста · экспорт и retest workflow',
        ctaLabel: 'Оплатить через OxaPay →',
        ctaHref: '/billing?product=team',
      },
      {
        name: 'Pro',
        price: '$599',
        priceAlt: '50 тестов / месяц',
        unit: '/мес',
        claim: 'Постоянное внешнее покрытие для растущих команд',
        bestFor:
          'Организациям, которым нужны больше активов, выше параллельность и регулярное обновление доказательств перед аудитом или релизом.',
        depth: '50 blackbox-тестов · 100 активов · 10 параллельных тестов · приоритетная валидация',
        ctaLabel: 'Оплатить через OxaPay →',
        ctaHref: '/billing?product=pro',
      },
      {
        ...
        claim: 'Ongoing offensive security coverage across products and teams',
        bestFor:
          'Security-командам с несколькими приложениями, репозиториями, требованиями SSO, retention и procurement-grade отчётностью.',
        depth: 'Blackbox + whitebox + PR review · SSO · retention controls · dedicated onboarding',
    ...
    faq: [
      ...
      },
      {
        ...
        a: 'Регион и retention фиксируются в правилах работы до запуска. Каждый проект изолирован: ключи, доказательства и журналы не пересекаются между заказчиками. По завершении работ доказательства удаляются по согласованному регламенту; журнал аудита и финальный отчёт хранятся в течение срока, указанного в договоре.',
      },
      {
      ...
      {
        q: 'Что входит в опубликованные тарифы?',
        a: 'Starter, Team и Pro покупают scan credits для self-serve blackbox-проверок через OxaPay. Whitebox, PR review и enterprise-программы считаются отдельно, потому что зависят от исходного кода, ролей, критичных потоков и правил валидации.',
      },
    ],
  ...
  wizard: {
    ...
    step4: {
      ...
      launching: 'Запускаем…',
      paidNotYet:
        'Платный запуск станет доступен после включения международного биллинга. Напишите нам для раннего доступа.',
      paidWithOxaPay:
        'Купите scan credits через OxaPay, затем вернитесь сюда и запустите проверку после начисления.',
