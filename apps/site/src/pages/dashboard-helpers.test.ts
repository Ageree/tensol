...
import type { BadgeMapping } from "./dashboard-helpers.ts";
import {
	FREE_TIER_WINDOW_MS,
...
describe("mapStatusToBadge", () => {
	const cases: readonly BadgeCase[] = [
		["draft", "muted", "draft"],
...
describe("mapStatusToAction", () => {
	...
	it("running → view / live", () => {
		expect(mapStatusToAction("running")).toEqual({
			key: "view",
			route: "live",
		});
	});
...
describe("deriveFreeQuotaStatus", () => {
	...
	it("failed free_quick is refunded → available", () => {
		const created = now - 1 * 24 * 60 * 60 * 1000;
		const q = deriveFreeQuotaStatus(
			[
				order({
