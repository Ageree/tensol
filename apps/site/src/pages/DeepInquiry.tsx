...
//   - If `GET /v1/auth/me` returns a session, prefill `email`.
//
// Wire contract:
...
function mapServerValidationDetails(
  ...
  for (const raw of details) {
    ...
    if (!field) continue;
    const msg =
      (typeof item.message === 'string' && item.message) || fallback;
    out[field] = msg;
  }
  return out;
}
...
export default function DeepInquiry(): ReactElement {
  ...
  // Best-effort prefill from /v1/auth/me. Anonymous users silently get null
