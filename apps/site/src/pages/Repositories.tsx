	...
	if (!editing) {
		return (
			<button
				...
				title={
					branches.length > 0 ? branches.join(", ") : t.branchesPlaceholder
				}
...
function LastReviewCell({
	lastReview,
	noReviewLabel,
}: LastReviewCellProps): ReactElement {
	if (!lastReview) {
		return (
	...
	return (
		...
		>
			...
			<td style={{ padding: "14px 8px", verticalAlign: "top", minWidth: 120 }}>
				<LastReviewCell
					lastReview={repo.last_review}
					noReviewLabel={t.lastReviewNone}
				/>
			</td>
...
export default function Repositories(): ReactElement {
	...
	useEffect(() => {
		...
		async function load(): Promise<void> {
			try {
				const { connected, installations } =
					await apiClient.github.installations();
				if (cancelled) return;
				...
				await forEachWithConcurrency(
					...
					async (inst) => {
						try {
							const repos = await apiClient.github.installationRepos(
						...
						<>
							...
							{state.installations.map((inst) => (
								<InstallationSection
									...
									reposLoading={
										inst.reposLoading ||
										state.disconnecting.has(inst.installation.id)
									}
