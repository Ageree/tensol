...
function NavItem({
  ...
  const style: CSSProperties = {
    ...
    background: active ? (hacktronLight ? 'var(--bg-alt)' : 'var(--fg)') : hov ? 'var(--bg-alt)' : 'transparent',
    color: active
      ? hacktronLight
        ? 'var(--fg)'
        : 'var(--bg)'
    ...
    textAlign: 'left',
    width: collapsed ? 'calc(100% - 16px)' : hacktronLight ? 'calc(100% - 20px)' : 'calc(100% - 24px)',
    margin: collapsed ? '0 8px 4px' : hacktronLight ? '0 10px 4px' : '0 12px 4px',
    position: 'relative',
...
export function AppShell({
  ...
  const shouldShowTopbar =
    (!isWhiteMono && !isHacktronLight) || actions != null || showLanguageSwitcher;
  ...
  return (
    <div
      ...
      style={{
        ...
        color: 'var(--ink)',
        display: 'grid',
        fontFamily: isHacktronLight ? '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif' : undefined,
    ...
    >
      {isHacktronLight && <style>{`${APP_SHELL_RESPONSIVE_CSS}\n${DASHBOARD_UI_CSS}`}</style>}
      <aside
      ...
      >
        <div
          ...
          style={{
            minHeight: 76,
            padding: sidebarCollapsed ? '18px 0 14px' : isHacktronLight ? '18px 18px 14px' : '20px 16px',
        ...
        >
          {!sidebarCollapsed && brand === 'sthrip' && isHacktronLight ? (
            ...
            >
              ...
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>sthrip</span>
              <span aria-hidden="true" style={{ color: 'var(--fg-3)', fontSize: 13 }}>⌄</span>
            </button>
        ...
        <div className="app-shell-nav-group" style={{ padding: isHacktronLight ? '0' : '12px 0 0' }}>
        ...
        <div className="app-shell-nav-group" style={{ padding: isHacktronLight ? '18px 0 0' : '24px 0 0' }}>
          {shouldShowSidebarLabels && (
            <div className="app-shell-group-eyebrow">
              <Eyebrow style={{ padding: '0 16px 8px', fontSize: 10 }}>{isHacktronLight ? 'Management' : t.navAccount}</Eyebrow>
            </div>
          )}
          <NavItem
      ...
      <div className="app-shell-content" style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        {shouldShowTopbar && (
          <header
        ...
        <main
          ...
          style={{
            padding: isHacktronLight ? 0 : density === 'compact' ? '20px 24px' : '32px 28px',
            flex: 1,
            minWidth: 0,
          }}
        >
