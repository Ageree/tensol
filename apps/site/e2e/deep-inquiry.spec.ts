 ...
 *
 * NOTE: like the other US2/US1 specs in this directory, running this file
 * requires the v2 test-server (T102 deliverable) to expose
 *   - POST /__test/v2/seed-session      { email } → { session_id, user_id }
 *   - POST /v1/deep-inquiries           (the real production endpoint)
 *   - GET  /v1/auth/me                  (the real production endpoint)
 *
 * Until then, this file documents the end-to-end assertions and type-checks
 * cleanly under `@playwright/test`.
 */
...
interface DeepInquiryFormInput {
  ...
  readonly budget_band?: '' | 'open' | 'under_500k' | '500k_1m' | '1m_3m' | '3m_plus';
}
...
async function fillDeepInquiryForm(
  ...
  // Consent checkbox (always required by server Zod).
  // primitives.Checkbox renders a native <input type="checkbox"> wrapped
  // in a label; targeting the role is the most stable selector.
  ...
  test('signed-in pre-fills contact_email from session', async ({
    ...
    try {
      ...
      await fillDeepInquiryForm(page, {
        ...
        // email left as-is — pre-filled by /v1/auth/me.
        phone: '@maratest',
