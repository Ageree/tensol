...
async function extractMagicLink(ctx: {
	...
	const VERIFY_URL_RE =
		/(https?:\/\/[^\s"<>]+\/api\/auth\/verify\?token=[A-Za-z0-9._%~\-]+)/;
	const apiContext = await ctx.request.newContext({
...
// Legacy 001 API quarantine: this spec still exercises removed
// /api/projects, /api/targets, /api/scans, and /webhooks/scan-progress
// surfaces. Current first-scan coverage lives in scan-wizard.spec.ts and the
// /v1/webhooks/scan-complete specs.
test.describe.skip("T090 — first scan end-to-end (legacy 001 API)", () => {
	...
	test("register → project → target → verify → scan → webhook → finding", async ({
		...
		const tgtRes = await api.post(`/api/projects/${projectId}/targets`, {
			headers: { "content-type": "application/json" },
		...
		const backendCtx = await pwRequest.newContext({
			baseURL: BACKEND_BASE_URL,
		});
		let signKey: string;
		try {
		...
		const webhookCtx = await pwRequest.newContext({
			baseURL: BACKEND_BASE_URL,
		});
		try {
		...
		const findingsCtx = await pwRequest.newContext({
			baseURL: BACKEND_BASE_URL,
		});
		try {
