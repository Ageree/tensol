...
async function mockAgentAccessApi(page: Page, tokenStatus = 200): Promise<void> {
  await page.route('**/v1/auth/me', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ id: 'user_1', email: 'user@example.com' }),
    }),
  );
...
test('settings keeps account visible when agent tokens fail and labels token input', async ({ page }) => {
...
test('reviews list does not coerce missing mode to fast', async ({ page }) => {
  ...
  await expect(page.getByRole('row', { name: new RegExp('acme/api') })).toContainText('deep');
  await expect(page.getByRole('row', { name: new RegExp('legacy/api') })).not.toContainText('fast');
  await expectNoPageOverflow(page);
});

test('review detail hides absent mode instead of rendering fast', async ({ page }) => {
  await useEnglish(page);
  await mockAgentAccessApi(page);
