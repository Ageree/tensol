...
test.describe("T120 — history re-download (US3)", () => {
	...
	test("completed scan in dashboard → report download → expire → regenerate", async ({
		...
		try {
			...
			// ─────────────────────────────────────────────────────────────────
			// 3. Visit /dashboard — assert the seeded row is present in the
			//    Organization activities table.
			// ─────────────────────────────────────────────────────────────────
			await page.goto("/dashboard");
			const scansTable = page.getByRole("region", {
			...
			// The action CTA for a `completed` order maps to the report
